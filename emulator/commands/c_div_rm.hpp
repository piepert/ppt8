...
public:
    ...
    int run(std::vector<uint8_t> args, Runtime* runtime) override // add_rm r, aa
    {
        ...
        runtime->getFX()->setValueComplete16bit(runtime->getRegisterByID(args[0])->getComplete16bit() % runtime->getMemory()->get16bitValueFromAddress(ppt8_std::combine_8bit_to_16bit(args[1], args[2])));
        return 0;
    }
};
